  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] text-ice-muted">
        <h2 className="text-2xl font-bold mb-4">Войдите, чтобы видеть подписки</h2>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto">
      <div className="flex items-center gap-3 mb-8">
        <Users className="w-6 h-6 text-ice-accent" />
        <h1 className="text-2xl md:text-3xl font-bold">Подписки</h1>
      </div>
          ...
          {channels.map(channel => (
            <Link key={channel.uid} to={`/channel/${channel.uid}`} className="flex flex-col items-center gap-2 min-w-[80px] group">
              <img 
                ...
                className="w-14 h-14 rounded-full border-2 border-ice-border group-hover:border-ice-accent transition-all" 
